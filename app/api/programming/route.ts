...
function parseProgramming(notionData: any[]) {
...
export async function GET() {
  try {
    if (!process.env.NOTION_TOKEN || !process.env.NOTION_PROGRAMMING_DB) {
      return NextResponse.json([]);
    }

    const data = await fetchNotionDatabase(process.env.NOTION_PROGRAMMING_DB);
    const programming = parseProgramming(data);
