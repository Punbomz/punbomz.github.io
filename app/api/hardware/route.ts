...
function parseHardware(notionData: any[]) {
...
export async function GET() {
  try {
    if (!process.env.NOTION_TOKEN || !process.env.NOTION_HARDWARE_DB) {
      return NextResponse.json([]);
    }

    const data = await fetchNotionDatabase(process.env.NOTION_HARDWARE_DB);
    const hardware = parseHardware(data);

    console.log("After", hardware)
